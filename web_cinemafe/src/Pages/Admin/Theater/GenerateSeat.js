...
const GenerateSeats = (props) => {
    ...
    const [generateSeatTypeReclining, setGenerateSeatTypeReclining] = useState(false);
        ...
        const updatedMatrix = seatMatrix.map((row, rIdx) => {
            if (rIdx === rowIndex) {
                let newRow = row.reduce((acc, seat, cIdx) => {
                    if (cIdx === colIndex) {
                        if (
                            getSeatTypeName(seatTypeList, seat.seatTypeId) === "Ðôi"
                            &&
                            (
                                getSeatTypeName(seatTypeList, seatTypeId) === "Đơn" ||
                                getSeatTypeName(seatTypeList, seatTypeId) === "Nằm" ||
                                seatTypeId === null
                            )
                        ) {
                            acc.push({
                            ...
                            (
                                getSeatTypeName(seatTypeList, seat.seatTypeId) === "Đơn" ||
                                getSeatTypeName(seatTypeList, seat.seatTypeId) === "Nằm" ||
                                seat.seatTypeId === null
                            )
                            &&
                            (
                            ...
                            if (
                                nextSeat &&
                                (
                                    getSeatTypeName(seatTypeList, nextSeat.seatTypeId) === "Đơn" ||
                                    getSeatTypeName(seatTypeList, nextSeat.seatTypeId) === "Nằm" ||
                                    nextSeat.seatTypeId === null
                                )
                            ) {
                                acc.push({
                                    ...seat,
    ...
    const generateSeats = () => {
        if (seatTypeList.length > 0) {
            ...
            setGenerateSeatTypeReclining(getSeatTypeName(seatTypeList, seatType) === "Nằm" ? true : false);
        }
    };


    const handleAddRowSeatType = (seatTypeId) => {
        const newCols = getSeatTypeName(seatTypeList, seatTypeId) === "Ðôi" ? cols / 2 : cols;
        const newRow = Array.from({ length: newCols }, (_, colIndex) => ({
            colIndex: colIndex + 1,
            rowName: String.fromCharCode(65 + seatMatrix.length),
            seatTypeId: seatTypeId,
            isSeat: false
        }));

        if (cols % 2 !== 0 && getSeatTypeName(seatTypeList, seatTypeId) === "Ðôi") {
            newRow.push({
                colIndex: newCols + 1,
                rowName: String.fromCharCode(65 + seatMatrix.length),
                seatTypeId: getSeatTypeId(seatTypeList, "Đơn"),
                isSeat: false
            });
        }

        setSeatMatrix([...seatMatrix, newRow]);
        setRows(rows + 1);
        onSeatsChange([...seatMatrix, newRow]);
        setHoveredRow(false);
    };

    const handleAddColumnSeatType = (seatTypeId) => {
    ...
    useEffect(() => {
        if (props.seats && props.seats.length > 0) {
            setSeatMatrix(props.seats.map(row => row.rowSeatsNew.length > 0 ? row.rowSeatsNew : row.rowSeats));
        }
    }, [props.seats]);
                        ...
                        props.config && (
                            <>
                                ...
                                <form onSubmit={(e) => { e.preventDefault(); generateSeats(); }}>
                                    ...
                                    <Button type="submit" variant="contained" color="primary" sx={{ mt: 1 }}>Tạo</Button>
                                </form>
                                {
                                    seatMatrix.length > 0 && (
                                            ...
                                            >
                                                Thêm dòng
                                                {
                                                    hoveredRow && <div style={{
                                                        ...
                                                        <Select
                                                            value={seatType}
                                                            onChange={(e) => handleAddRowSeatType(e.target.value)}
                                            ...
                                            >
                                                Thêm cột
                                                {
                                                    hoveredColumn && <div style={{
                                                        ...
                                                        <Select
                                                            value={seatType}
                                                            onChange={(e) => handleAddColumnSeatType(e.target.value)}
                                                            onClick={(e) => e.stopPropagation()}
                                                            displayEmpty
                                                            style={{ width: '100px' }}
                                                        >
                            ...
                            return (
                                <div key={rowIndex} style={{ marginBottom: gap, display: 'flex', gap, justifyContent: 'center' }}>
                                    {row.map((seat, colIndex) => {
                                        const seatName = seat.isSeat && seat.seatTypeId !== null ? `${seat.rowName}${colIndexText++}` : '';
                                        return (
                                            <div
                                                key={colIndex}
                                            ...
                                            >
                                                ...
                                                {hoveredSeat && hoveredSeat.rowIndex === rowIndex && hoveredSeat.colIndex === colIndex && props.config && (
                                                    <div style={{
                                                        position: 'absolute',
                                                        top: '100%',
                                                        left: '50%',
                                                        ...
                                                        >
                                                            <MenuItem value={null}>trống</MenuItem>
                                                            {seatTypeList.map((seat) => (
                                                                (!generateSeatTypeReclining && seat.name !== "Nằm") || (generateSeatTypeReclining && (seat.name === "trống" || seat.name === "Nằm")) ? (
                                                                    <MenuItem key={seat.id} value={seat.id}>
                                                                        {seat.name}
                                                                    </MenuItem>
                                                                ) : null
