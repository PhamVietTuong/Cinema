...
export const MovieListAdminAction = () => {
    return async (dispatch) => {
        try {
            const result = await cinemasService.GetMovieListAdmin();
            dispatch({
                type: SET_MOVIE_LIST,
                movieList: result.data,
            })
            ...
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log("GetMovieListAdminAction: ", error);
                }
            });
        }
    }
...
export const MovieByIdActionAdmin = (id) => {
    return async (dispatch) => {
        try {
            const result = await cinemasService.GetMovieByIdAdmin(id);
            ...
            console.log("GetMovieByIdAdmin", errors);
        }
    }
}


export const CreateMovieAction = (movieDTO) => {
    return async (dispatch) => {
        try {
            const result = await cinemasService.CreateMovie(movieDTO);
            ...
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log("CreateMovieAction: ", error);
                }
            });
        }
    }
...
export const GetRevenueAction = (filterRevenue) => {
    return async (dispatch) => {
        try {
            const result = await cinemasService.GetRevenue(filterRevenue);
            dispatch({
                type: SET_DATA_STATICTICAL,
                dataRevenue: result.data,
            })
        } catch (error) {
...
export const GetListInvoiceAction = (code) => {
    return async (dispatch) => {
        try {
            const result = await cinemasService.GetListInvoice(code);
            dispatch({
                type: SET_LIST_INVOICE,
                listInvoice: result.data,
            })
        } catch (error) {
            await Swal.fire({
                padding: "24px",
